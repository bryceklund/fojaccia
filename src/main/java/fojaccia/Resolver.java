...
public class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  public Void visitClassStmt(Stmt.Class stmt) {
    ...
    if (stmt.superclass != null) {
      if (stmt.superclass.name.lexeme.equals(stmt.name.lexeme))
        Fojaccia.Error(stmt.superclass.name, "A class can't inherit from itself");
  ...
  public Void visitVariable(Expr.Variable expr) {
    if (!scopes.isEmpty() && scopes.peek().get(expr.name.lexeme) == Boolean.FALSE) {
      Fojaccia.Error(expr.name, "Can't read local variable in its own initializer");
    }
  ...
  @Override
  public Void visitAssignment(Expr.Assignment expr) {
    resolve(expr.value);
    resolveLocal(expr, expr.name);
    return null;
  }
  ...
  public Void visitSuper(Expr.Super expr) {
    if (currentClass == ClassType.NONE) {
      Fojaccia.Error(expr.keyword, "Can't use `super` outside of a class");
    } else if (currentClass != ClassType.SUBCLASS) {
      Fojaccia.Error(expr.keyword, "Can't use `super` without a superclass");
    }
  ...
  public Void visitThis(Expr.This expr) {
    if (currentClass == ClassType.NONE) {
      Fojaccia.Error(expr.keyword, "Can't use `this` outside of a class");
      return null;
    }
    resolveLocal(expr, expr.keyword);
    return null;
  ...
  public Void visitReturnStmt(Stmt.Return stmt) {
    ...
    if (stmt.value != null) {
      if (currentFunction == FunctionType.INITIALIZER) {
        Fojaccia.Error(stmt.keyword, "Can't return a value from an initializer");
      }
      resolve(stmt.value);
    }
